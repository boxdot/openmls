...
impl OpenMlsCrypto for CryptoProvider {
    fn supports(&self, ciphersuite: Ciphersuite) -> Result<(), CryptoError> {
        ...
        // `hpke_aead` maps every one of these onto an algorithm that
        // `hpke-rs-libcrux` implements, and `libcrux-aead` compiles all of them
        // in via its default features. Matched exhaustively without a wildcard
        // so that a new `HpkeAeadType` variant is a compile error here rather
        // than a ciphersuite that silently stops being supported.
...
mod tests {
    ...
    /// `supported_ciphersuites()` must not advertise anything `supports()` then
    /// rejects. These drifted apart once already: the list contained
    /// `MLS_128_DHKEMX25519_AES128GCM_SHA256_Ed25519` — MLS's mandatory-to-
    /// implement ciphersuite — while `supports()` required a ChaCha20Poly1305
    /// HPKE AEAD and turned it down. Anything iterating the advertised list then
    /// panicked.
    #[test]
    ...
    /// The stronger claim: every advertised ciphersuite can actually complete the
    /// primitives MLS needs from it. `supports()` agreeing with the list is not
    /// worth much if the underlying algorithm is missing.
    #[test]
    fn advertised_ciphersuites_actually_work() {
        ...
        for ciphersuite in provider.supported_ciphersuites() {
            ...
            let ciphertext = provider
                .aead_encrypt(ciphersuite.aead_algorithm(), &key, b"plaintext", &nonce, b"aad")
